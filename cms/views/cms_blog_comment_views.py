...
def getBlogCommentsByTitle(request, title):
    try:
        ...
        content = BlogComments.objects.get(title=title)
        
     
        serializer = BlogCommentsSerializer(content)
        ...
        return Response({'detail': f"BlogComments by title '{title}' does not exist"}, 
